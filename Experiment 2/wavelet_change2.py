...
def wt(vimg):
 ...
 # original   res=torch.zeros(vimg.shape[0],4*vimg.shape[1],int(vimg.shape[2]/2),int(vimg.shape[3]/2))
  ...
  #second level of WT
        ...
        temp_pad[:,:,1:res.shape[2]+1,1:res.shape[3]+1]=res[:,8*i:8*i+1,:,:]
        res[:,8*i+4:8*i+8] = torch.nn.functional.conv2d(temp_pad, Variable(filters[:,None].cuda(),requires_grad=True),stride=1)
        #original res[:,4*i+1:4*i+4]=(res[:,4*i+1:4*i+4]+1)/2.0
        res[:,8*i+5:8*i+8]=(res[:,8*i+5:8*i+8]+1)/2.0
